...
def graph_chart(chart, df):
        ...
        if chart.lower() == 'candlestick':
            fig = go.Figure(data=[go.Candlestick(x = df.index, name=f'{tickerSymbol} Price', open=df['Open'], high=df['High'], low=df['Low'], close=df['Close'])])
        ...
        fig.add_trace(go.Scatter(x = df.index, y=df[f'{strat} {SMA1}'], mode='lines', name= f'{strat} {SMA1}'))
        fig.add_trace(go.Scatter(x = df.index, y=df[f'{strat} {SMA2}'], mode='lines', name= f'{strat} {SMA2}'))
